...
async function consultarVentas (req, res) {
  ...
  try {
    ...
    const idsucursal = await pool.query(
      'select idsucursal from usuarios where idUsuario=?',
      [idusuario]
    )
    if (tipo === 'codigo') {
      sql =
        'SELECT a.*, t.tarifa as tarifa,p.valor as valor, i.cantidad as stock FROM articulos a, tarifasiva t, precios p, inventario i ' +
        'WHERE t.idtarifaiva = a.idtarifaiva AND p.idarticulo = a.idarticulo AND i.idarticulo = a.idarticulo AND i.idbodega=(SELECT id FROM bodegas where ?) AND p.idlistaprecios = ? AND a.codigo = ?'
      params = [idsucursal[0], idlistaprecios, dato]
    } else {
      sql =
