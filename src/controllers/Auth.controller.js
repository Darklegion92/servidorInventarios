...
async function crear (req, res) {
  ...
  const { usuario, password, nombre, sucursal, idusuario } = req.body
  try {
    if (password) {
...
async function actualizar (req, res) {
  ...
  const { usuario, password, nombre, sucursal, idusuario } = newUser
  try {
    let sql = 'UPDATE usuarios set ? where idusuario=?'
    let parametros = [
      { usuario, asignacion: nombre, idsucursal: sucursal, password },
      user.idusuario
    ]

    if (!password && user.nombresucursal === sucursal) {
      parametros = [{ usuario, asignacion: nombre }, user.idusuario]
    } else if (!password) {
      parametros = [
        { usuario, asignacion: nombre, idsucursal: sucursal },
        user.idusuario
      ]
    } else if (user.idnombresucursal === sucursal) {
      parametros = [{ usuario, asignacion: nombre, password }, user.idusuario]
    }
